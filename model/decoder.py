...
class TransformerDecoderLayer(nn.Module):
    ...
    def _forward(self, inputs, memory_bank, src_pad_mask, tgt_pad_mask,
        ...
        # server bug
        # RuntimeError: Given normalized_shape=[256], expected input with shape [*, 256], but got input of size[4, 10, 500]
...
class TransformerDecoder(DecoderBase):
    ...
    def forward(self, tgt, memory_bank, step=None, **kwargs):
        ...
        # output [2,13,256]
        if 'teacher_outputs' in kwargs:
            output = kwargs['teacher_outputs'].transpose(0,1).contiguous()
        else:
            output = emb.transpose(0, 1).contiguous()

        src_memory_bank = memory_bank.transpose(0, 1).contiguous()
        # src_memory_bank [2,23,256]
