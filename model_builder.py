...
import model.nmtmodel as custom_NMTmodel
import model.decoder as custom_decoder
...
def build_base_model(model_opt, fields_opt, gpu, checkpoint=None, gpu_id=None):
    ...
    original_field = fields_opt['original']
    teacher_field = None

    if 'teacher' in fields_opt:
        teacher_field = fields_opt['teacher']

    # Build embeddings.
    if model_opt.model_type == "text" or model_opt.model_type == "vec":
        src_field = original_field['src']
    ...
    if teacher_field is None:
        tgt_field = original_field['tgt']
        logger.info("using original tgt vocab to build decoder.")
    else:
        tgt_field = teacher_field['tgt']
        logger.info("using teacher tgt vocab to build decoder")
    ...
    #decoder = custom_decoder.TransformerDecoder.from_opt(model_opt, tgt_emb)
    ...
    model = custom_NMTmodel.NMTmodel(encoder, decoder)
...
def build_model(model_opt, opt, fields_opt: dict, checkpoint):
    logger.info('Building custom model...')
    model = build_base_model(model_opt, fields_opt, use_gpu(opt), checkpoint)
    logger.info(model)
    return model
