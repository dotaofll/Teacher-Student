...
def build_loss_compute(model, teacher_model, tgt_field, opt, train=True):
    ...
    if opt.copy_attn:
        ...
        compute = NMTLossCompute(
            criterion, loss_gen, False, teacher_loss_gen)
...
class MyLoss(onmt.utils.loss.LossComputeBase):
    def __call__(self, batch, output, attns, normalization=1.0, shard_size=0, trunc_start=0, trunc_size=None, teacher_outputs=None):
        ...
        shard_state = self._make_shard_state(
            batch, output, trunc_range, attns, teacher_outputs=teacher_outputs)
        if shard_size == 0:
...
class NMTLossCompute(MyLoss):
    def __init__(self, generator, criterion, use_distillation_loss=False, teacher_generator=None):
    ...
    def _make_shard_state(self, batch, output, range_, attns=None, teacher_outputs=None):
        res_dict = {}
    ...
    def _compute_loss(self, batch, output, target, **kwargs):
        ...
        if __debug__:
            print('using criterion func: {name}'.format(
                name=self._criterion.__class__.__name__))
