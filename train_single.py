...
import model_builder as custom_builder
import model_saver as custom_model_saver
from onmt.inputters.inputter import (build_dataset_iter,
                                     build_dataset_iter_multiple,
                                     load_old_vocab, old_style_vocab)
from onmt.model_builder import build_model
from onmt.models import build_model_saver
from trainer.onmt_trainer import build_trainer, MyTrainer
...
from loss.loss import NMTLossCompute
...
def main(opt, device_id, batch_queue=None, semaphore=None):
    ...
    teacher_model = build_model(
        teacher_model_opt, teacher_model_opt, teacher_fields, teacher_model_ckpt) if opt.teacher_model_path else None
    ...
    model_saver = custom_model_saver.build_model_saver(model_opt, opt, model, fields_opt, optim)
    ...
    if teacher_model is not None and opt.word_sampling:
        sampler = Emulator(teacher_model, teacher_fields, device_id, max_length=50, random_sampling_topk=5)
    else:
        sampler = None

    if teacher_model is not None:
        trainer = build_trainer(opt, device_id, model,
                                teacher_fields, optim, model_saver, teacher_model=teacher_model, emulator=sampler)
    else:
        trainer = build_trainer(opt, device_id, model,
                            fields, optim, model_saver, teacher_model=teacher_model, emulator=sampler)
...
def main(opt, device_id, batch_queue=None, semaphore=None):
    ...
    if opt.teacher_model_path:
        teacher_model = build_model(
            teacher_model_opt, teacher_model_opt, teacher_fields, teacher_model_ckpt)
    ...
    trainer = build_trainer(opt, device_id, model,
                            teacher_model, fields, optim, model_saver)
    ...
    if trainer.report_manager.tensorboard_writer is not None:
        trainer.report_manager.tensorboard_writer.close()
>>>>>>> 7373cbfbe38909a85a7d2de9d918dd4de6c7b188
